%% =========================================================
%  learn_koopman_dcmotor.m   — CDC 2026 KOT tutorial paper
%
%  PART 1 of 2.  Trains four Koopman model structures on a DC
%  motor with input nonlinearity (Haseli-Cortes 2023, Example 9.1):
%
%     d/dt x1 = -(Ra/La) x1 - (km/La) x2 f(u) + ua/La
%     d/dt x2 = -(B /J ) x2 + (km/J ) x1 f(u) - tau_l/J
%
%  with f(u) either:
%     1) saturated      f(u) = 2 tanh(u)
%     2) non-monotone   f(u) = 2 tanh(u cos(u))
%
%  The dynamics are simulated in PHYSICAL coordinates; training,
%  lifting, and Koopman fitting are done in SCALED coordinates
%    x_scaled = Sx * x_physical,    u_scaled = Su * u_physical.
%  The linear decoder D maps scaled lifted features back to scaled
%  states; mpc Part 2 converts to physical units for plotting.
%
%  Reference equations (z = phi_x(x_scaled), v = phi_u(u_scaled)):
%    1. Linear     :  z+ = A z + B u
...
%
%  Output : koopman_dcmotor_model_<kernel>.mat  (loaded by Part 2)
%% =========================================================
...
%   1 : saturated      f(u) = 2 tanh(u)
%   2 : non-monotone   f(u) = 2 tanh(u cos(u))
INPUT_NONLINEARITY = 2;

% --- DC motor parameters (physical) ---
params.Ra    = 12.345;
params.La    = 0.314;
params.km    = 0.253;
params.ua    = 60;
...
params.tau_l = 1.47;
params.J     = 0.00441;
params.Ts    = 0.005;    % 5 ms

% --- Coordinate scaling ---
%   Scale each channel by ~ 1/(its peak magnitude) so the scaled data lands in
%   roughly [-1, 1]. This puts x1 (~10 A) and x2 (~100s rad/s) on a comparable
%   footing -- so one kernel/poly basis and one gamma fit both -- and puts u in
%   [-1, 1] for the Chebyshev input basis. The decoder and MPC undo the scaling,
%   so it affects conditioning, not the result.
params.Sx     = diag([0.1, 0.004]);   % x_scaled = Sx * x_physical (x1~10A, x2~250 rad/s)
params.Su     = 0.25;                  % u_scaled = Su * u_physical (|u|<=4 -> [-1,1])
params.Sx_inv = inv(params.Sx);
% Resolve nonlinearity handle (applied in PHYSICAL u)
switch INPUT_NONLINEARITY
    case 1
        params.f_name = 'saturated  f(u) = 2 tanh(u)';
        params.f_fun  = @(u) 2 * tanh(u);
    case 2
        params.f_name = 'non-monotone  f(u) = 2 tanh(u cos(u))';
        params.f_fun  = @(u) 2 * tanh(u .* cos(u));
    otherwise
        error('INPUT_NONLINEARITY must be 1 or 2.');
end
...
% --- Data collection ---
%   L  = number of training trajectories; sample physical ICs from a
%        wide box around the motor's natural operating range.
%   Smaller L + M_raw + M_target acts as implicit regularization for the
%   multi-step solve, which otherwise over-inflates input-coupling operator
%   norms (||N||, ||A_12||) and produces oscillating rollout errors and
%   erratic MPC inputs. 
L         = 6;
T_long    = 2000;
M_raw_max = 6000;
...
% gamma : Tikhonov (ridge) regulariser. Every Koopman form below is fit by a
%   regularised least squares,  Theta = (Zp*Phi') / (Phi*Phi' + gamma*I).
%   The lifted regressors are correlated (degree-3 monomials, and the nz*nv
%   Kronecker space for GeKo/KCF), so Phi*Phi' is near-singular; the gamma*I
%   term makes the inverse well-defined and well-conditioned. It trades a
%   little bias for much lower variance, keeping operator norms bounded and
%   rollouts stable. Larger gamma => smoother, more damped operators;
%   gamma -> 0 recovers the unregularised (often unstable) fit.
gamma     = 1e-4;

% --- Open-loop validation ---
T_test  = 300;
x0_test_phys = [3; -40];  % representative IC (also IC #1 in validation set)
N_test  = 10;
...
if ~ismember(INPUT_NONLINEARITY, 1:2)
    error('INPUT_NONLINEARITY must be 1 or 2 (got %d).', INPUT_NONLINEARITY);
end
must_pos = struct( ...
...
if L < 3
    error('L must be >= 3 (got %d).', L);
end
if N >= T_long
    error('N (=%d) must be < T_long (=%d).', N, T_long);
end
if M_target > M_raw_max
    warning('M_target (%d) > M_raw_max (%d).', M_target, M_raw_max);
end
if numel(x0_test_phys) ~= 2
    error('x0_test_phys must be a 2-element vector.');
end
...
save_path = sprintf('koopman_dcmotor_model_%s.mat', ktag);
save_path_probe = save_path;
[fid, msg] = fopen(save_path_probe, 'a');
if fid == -1
    error('Cannot write ''%s'':\n   %s', save_path_probe, msg);
else
    fclose(fid);
    info = dir(save_path_probe);
    if ~isempty(info) && info.bytes == 0, delete(save_path_probe); end
end
...
% Build the KCF coupling index from the degree setting (polynomial lift only;
% Section 8b sweeps the degree).
% Validate KCF coupling settings. The index vector itself is built after
% Z_all (Section 5), because the kernel-lift variance ranking needs the
...
fprintf('\n=========================================\n');
fprintf(' Koopman Form Comparison — DC motor (Part 1)\n');
fprintf(' Nonlinearity : %s\n', params.f_name);
fprintf(' State kernel : %s\n', kernel_name_x);
fprintf(' Input kernel : %s\n', kernel_name_u);
fprintf(' nz=%d  nv=%d  Ts=%.3f s\n', nz, nv, params.Ts);
fprintf('=========================================\n');

%% =========================================================
%  SECTION 1 : GENERATE TRAINING TRAJECTORIES
%
%  Physical ICs sampled from a box around the motor's natural
%  operating region; physical input is a broadband multi-sine.
%  Both are scaled to (x_scaled, u_scaled) before lifting.
%% =========================================================
...
x2_box = [-80, 20];
for l = 1:L
    x0_l_phys = [x1_box(1) + diff(x1_box)*rand;
                 x2_box(1) + diff(x2_box)*rand];
    t_vec  = (0:T_long-1) * params.Ts;
    freqs  = [0.3, 0.7, 1.1, 1.9, 3.1, 5.3] * 5;   % motor is fast
    phases = 2*pi*rand(1,6);
    u_l_phys = sum(0.8 * sin(freqs .* t_vec' + phases), 2)';
    u_l_phys = max(-4, min(4, u_l_phys));
    X_l_phys = simulate_dcmotor(x0_l_phys, u_l_phys, params);
    X_l = params.Sx * X_l_phys;        % scaled state
    u_l = params.Su * u_l_phys;        % scaled input
    X_trajs{l} = X_l;  U_trajs{l} = u_l;
...
%% =========================================================
%  SECTION 2 : KERNEL CENTRES (state only)
%% =========================================================
fprintf('\n[2] Kernel centres...\n');
if use_centres_x
...
%% =========================================================
%  SECTION 4 : COLLECT MULTI-STEP WINDOWS  (cells, then preallocate)
%% =========================================================
...
%% =========================================================
%  SECTION 4b : K-MEANS WINDOW SELECTION
%
...
%  multi-step fit sees residuals spread over distinct regions of
%  the lifted flow z_0..z_N rather than near-duplicate rollouts
...
M_act = numel(sel);
M_aug = N * M_act;
fprintf('   Selected windows: %d   Multi-step pairs: %d\n', M_act, M_aug);

%% =========================================================
%  SECTION 4c : BUILD SHARED DESIGN MATRICES
%
%  Multi-step pairs (z_k, v_k, u_k, z_{k+1}) come from k=0..N-1
%  columns of the M_act selected windows -- M_aug = N * M_act pairs.
%
...
%% =========================================================
%  SECTION 5 : LINEAR DECODER  D : z -> x_scaled
%% =========================================================
...
%
%  Frobenius norms are a DIAGNOSTIC, not an accuracy score -- read them
%  RELATIVELY within one model. Which block dominates says how the input acts
%  (additive ||B|| vs state-modulated ||N|| / ||A_12||); an input-coupling
%  norm that dwarfs the autonomous block flags an over-inflated, ill-
%  conditioned fit that tends to produce oscillatory rollouts and erratic MPC
%  inputs (raise gamma or shrink the data set). Comparing the MULTI-STEP vs
%  ONE-STEP tables shows whether multi-step training tamed those norms.
%  Absolute values depend on lift scaling (sqrt(2/nz), Sx, Su), so don't
%  compare across kernels or benchmarks.
%
%    Linear   :  ||A|| autonomous, ||B|| input gain
%    Bilinear :  ||A|| autonomous, ||B|| input, ||N|| state-modulated input
%    GeKo     :  ||K|| combined operator (no natural decomposition)
%    KCF      :  ||A_11|| autonomous, ||A_12|| input-modulated
%% =========================================================
...
% Test input (physical)
freqs_t  = [0.4, 1.0, 2.3, 4.7] * 5;
phases_t = 2*pi*rand(1,4);
u_test_phys = sum(0.8 * sin(freqs_t .* t_ax(1:T_test)' + phases_t), 2)';
u_test_phys = max(-4, min(4, u_test_phys));
u_test = params.Su * u_test_phys;

% Test ICs: IC #1 = x0_test_phys fixed; rest random in physical box
rng_state_ic = rng;
rng(2025, 'twister');
X0_test_phys           = zeros(2, N_test);
X0_test_phys(:,1)      = x0_test_phys;
X0_test_phys(1,2:end)  = x1_box(1) + diff(x1_box)*rand(1, N_test-1);
X0_test_phys(2,2:end)  = x2_box(1) + diff(x2_box)*rand(1, N_test-1);
rng(rng_state_ic);

V_test = cheb_lift_input(u_test, deg_cheb_u);
...
X_test_rep_phys = [];  % IC #1 ground truth (physical), for trajectory plots

for i = 1:N_test
    X_i_phys = simulate_dcmotor(X0_test_phys(:,i), u_test_phys, params);
    X_i      = params.Sx * X_i_phys;
    if i == 1, X_test_rep_phys = X_i_phys; end
    if use_centres_x
    ...
    for m = 1:n_models
        pred_fn = models{m, 3};
        Z_hat_ms = rollout(M_ms{m}, pred_fn, Z_true_i(:,1), V_test, u_test, nz, T_test);
        Z_hat_1s = rollout(M_1s{m}, pred_fn, Z_true_i(:,1), V_test, u_test, nz, T_test);
...
for m = 1:n_models
    mae_x_ms(:, m) = mean(err_x_all_ms{m}, 2);
    mae_x_1s(:, m) = mean(err_x_all_1s{m}, 2);
end

fprintf('\n   Per-trajectory aggregated error (scaled state, MULTI-STEP)\n');
...
fprintf('\n   Per-trajectory aggregated error (scaled state, ONE-STEP)\n');
fprintf('     mean_t ||x_hat - x*||,  averaged over T+1 = %d steps.\n', T_test+1);
fprintf('     (bold value = lowest of the 4 forms for that IC)\n');
fprintf('     %-3s', 'IC');
for m = 1:n_models, fprintf('  %12s', models{m,1}); end
...
for i = 1:N_test
    Xi_phys = simulate_dcmotor(X0_test_phys(:,i), u_test_phys, params);
    Xi      = params.Sx * Xi_phys;
    if use_centres_x
...
for k = 1:numel(budgets)
    ...
    e_ic = zeros(N_test, 1);
    for i = 1:N_test
        Z_hat   = rollout(Mq, @predict_kcf, z0gt(:,i), V_test, u_test, nz, T_test);
        X_hat   = D_lin * Z_hat;
...
% --- Figure 1: feature-space error vs time (IC #1) ---
...
title(sprintf('Feature error (IC #1)  |  multi-step  |  %s  |  %s', ...
    kernel_name_x, params.f_name));
...
% --- Figure 2: state-space (scaled) error vs time (IC #1) ---
...
set(gca,'YScale','log');
xlabel('Time [s]');  ylabel('|x_{hat} - x*|  (scaled)');
...
% --- Figure 3: state trajectories (PHYSICAL units) for IC #1 ---
% Recompute multi-step predictions and decode back to physical.
X_hat_all_phys = cell(n_models, 1);
if use_centres_x
    z0_rep = lift_state(params.Sx * X_test_rep_phys(:,1), C_x, hp_x, kfun_x, prepend_state);
else
    z0_rep = poly_lift_state(params.Sx * X_test_rep_phys(:,1), deg_poly_x);
end
for m = 1:n_models
    Z_hat = rollout(M_ms{m}, models{m,3}, z0_rep, V_test, u_test, nz, T_test);
    X_hat_scaled = D_lin * Z_hat;
    X_hat_all_phys{m} = params.Sx_inv * X_hat_scaled;
end

figure('Name','Figure 3: state trajectories (IC #1, physical)');
subplot(2,1,1);  hold on;
plot(t_ax, X_test_rep_phys(1,:), 'k-', 'LineWidth', 2.5);
for m = 1:n_models
    plot(t_ax, X_hat_all_phys{m}(1,:), '-', 'Color', colors(m,:), 'LineWidth', 1.5, ...
        'Marker', markers{m}, 'MarkerIndices', mk_idx(numel(t_ax)), 'MarkerSize', 5);
end
ylabel('x_1  (current, A)');
title(sprintf('Open-loop state prediction (IC #1)  |  %s  |  %s', ...
    kernel_name_x, params.f_name));
legend([{'True'}; models(:,1)], 'Location', 'best');  grid on;
subplot(2,1,2);  hold on;
plot(t_ax, X_test_rep_phys(2,:), 'k-', 'LineWidth', 2.5);
for m = 1:n_models
    plot(t_ax, X_hat_all_phys{m}(2,:), '-', 'Color', colors(m,:), 'LineWidth', 1.5, ...
        'Marker', markers{m}, 'MarkerIndices', mk_idx(numel(t_ax)), 'MarkerSize', 5);
end
ylabel('x_2  (angular vel, rad/s)');  xlabel('Time [s]');
legend([{'True'}; models(:,1)], 'Location', 'best');  grid on;

% --- Figure 4: phase portrait (PHYSICAL units) for IC #1 ---
figure('Name','Figure 4: phase portrait (IC #1, physical)');
hold on;
plot(X_test_rep_phys(1,:), X_test_rep_phys(2,:), 'k-', 'LineWidth', 2.5);
for m = 1:n_models
    plot(X_hat_all_phys{m}(1,:), X_hat_all_phys{m}(2,:), '-', ...
        'Color', colors(m,:), 'LineWidth', 1.5, ...
        'Marker', markers{m}, 'MarkerIndices', mk_idx(size(X_hat_all_phys{m},2)), 'MarkerSize', 5);
end
% Starting IC marker
plot(X_test_rep_phys(1,1), X_test_rep_phys(2,1), 'ks', ...
    'MarkerSize', 10, 'MarkerFaceColor', 'y');
xlabel('x_1  (A)');  ylabel('x_2  (rad/s)');
title(sprintf('Phase portrait (IC #1)  |  %s', kernel_name_x));
legend([{'True'}; models(:,1); {'x_0'}], 'Location', 'best');
grid on;
...
% (e.g. koopman_dcmotor_model_poly.mat).
% Note: function handles to local functions don't survive .mat reliably.
% Part 2 reconstructs handles from KERNEL_TYPE and INPUT_NONLINEARITY.
model_names = models(:, 1);
% Drop the anonymous nonlinearity handle before saving: anonymous handles do
% not reload cleanly in another script (they warn "Could not find appropriate
% function on path"). Part 2 rebuilds f_fun from INPUT_NONLINEARITY; only the
% integer flag and the f_name string are needed downstream.
if isfield(params, 'f_fun'), params = rmfield(params, 'f_fun'); end
save(save_path, ...
    'M_ms', 'M_1s', 'model_names', ...
    'KERNEL_TYPE', 'INPUT_NONLINEARITY', ...
    'kernel_name_x', 'kernel_name_u', ...
    'use_centres_x', 'C_x', 'hp_x', ...
    'deg_poly_x', 'deg_cheb_u', ...
    'nz', 'nv', 'params', 'D_lin', 'prepend_state', '-v7.3');
fprintf('   Saved to %s\n', save_path);
fprintf('\nPart 1 complete.\n');
...
% ---------- DC motor simulator (RK45) ----------
function X = simulate_dcmotor(x0, u_seq, p)
    T = numel(u_seq);  X = zeros(2, T+1);  X(:,1) = x0(:);
    opts = odeset('RelTol',1e-8,'AbsTol',1e-10);
    for t = 1:T
        ut = u_seq(t);  fu = p.f_fun(ut);
        f = @(~,x)[ -(p.Ra/p.La)*x(1) - (p.km/p.La)*x(2)*fu + p.ua/p.La;
                    -(p.B /p.J )*x(2) + (p.km/p.J )*x(1)*fu - p.tau_l/p.J ];
